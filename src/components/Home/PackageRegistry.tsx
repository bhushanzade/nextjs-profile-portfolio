        ...
        <section className="relative z-10 container mx-auto px-4 py-24">
            ...
            <div className="flex flex-col items-center mb-24 relative">
                <div className="relative group cursor-default">
                    <h2 className="text-4xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white via-neon-cyan to-white uppercase tracking-tighter opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                ...
                <div className="mt-4 flex items-center gap-4 text-xs font-mono text-neon-cyan/70 tracking-[0.3em]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 relative z-10 perspective-2000">
                {packages.map((pkg, idx) => (
                    ...
                    >
                        ...
                        <div className="absolute -top-10 left-1/2 w-0.5 h-10 bg-gradient-to-b from-transparent to-neon-cyan/20 -z-10 group-hover:h-12 group-hover:-top-12 transition-all duration-300 pointer-events-none" />
                        <div className="absolute -top-12 left-1/2 w-2 h-2 rounded-full bg-neon-cyan/0 group-hover:bg-neon-cyan/50 blur-sm transition-all duration-300 -translate-x-1/2 pointer-events-none" />

                        {/* Holographic Card Logic */}
                        <GlassCard className="h-full relative overflow-hidden bg-black/40 border-white/5 group-hover:border-neon-cyan/50 group-hover:bg-black/60 transition-all duration-500 backdrop-blur-md rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] group-hover:shadow-[0_0_30px_rgba(0,243,255,0.15)]">
                            ...
                            <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <rect x="0" y="0" width="100%" height="100%" fill="none" rx="12" ry="12" stroke="url(#gradient-border)" strokeWidth="2" strokeDasharray="400" strokeDashoffset="400">
                            ...
                            <div className="p-8 h-full flex flex-col relative z-20">
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex flex-col">
                                        <span className="text-[9px] text-neon-cyan font-mono tracking-widest uppercase mb-1 opacity-70">Module Type</span>
                                        <span className="text-xs text-white font-bold bg-white/10 px-2 py-1 rounded">{pkg.type}</span>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[9px] text-neon-purple font-mono tracking-widest uppercase mb-1 opacity-70">Version</span>
                                        <span className="text-xs text-neon-purple font-mono">{pkg.version}</span>
                                    </div>
                                </div>

                                {/* Package Identity */}
                                <h3 className="text-2xl font-bold text-white mb-4 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-neon-cyan group-hover:to-white transition-all duration-300">
                                    {pkg.name}
                                </h3>

                                {/* Description with Decrypt Effect on Hover */}
                                <div className="text-sm text-gray-400 leading-relaxed mb-8 flex-grow h-20 overflow-hidden font-mono">
                                    <DecryptText text={pkg.desc} reveal={hoveredPkg === pkg.id} />
                                </div>

                                {/* Terminal Install Simulation */}
                                <div className="mt-auto relative overflow-hidden rounded bg-black/80 border border-white/10 p-3 mb-4 group-hover:border-neon-cyan/30 transition-colors">
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-white/5 group-hover:border-neon-cyan/20 transition-colors">
                                    <div className="flex flex-col">
                                        <span className="text-[9px] uppercase text-gray-500 font-mono">Downloads</span>
                                        <span className="text-lg font-bold text-white">{pkg.downloads}</span>
                                    </div>
                                    <a href={pkg.url} target="_blank" rel="noopener noreferrer">
                                        <NeonButton className="px-5 py-2 text-xs group-hover:shadow-[0_0_20px_rgba(0,243,255,0.4)] transition-shadow">
                                            Access Node
                                        </NeonButton>
                                    </a>
                                </div>
